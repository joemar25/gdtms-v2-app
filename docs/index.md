...
1. **When you edit a source file** → open its matching doc file (the `DOCS:` comment in the file header tells you which one) and update it.
2. **When you add a new file or folder** → create a matching doc file, add it to this index, and add it to the `README.md` Documentation section.
3. **When you remove a file** → remove or update its doc entry here and in `README.md`.
